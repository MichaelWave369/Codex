...
class CodexTIEKATReport:
    ...
    def high_confidence_matches(self) -> list[PatternMatch]:
        return [m for m in self.matches if m.confidence in (Confidence.HIGH, Confidence.STRUCTURAL)]
...
class TIEKATPatternEngine:
    ...
    def analyze(
        ...
        for pattern_defs, pattern_type in [
            ...
            lexical = _scan_lexical_patterns(text, pattern_defs, pattern_type, self.tradition_label)
        ...
        def density(pt: PatternType) -> float:
            hits = sum(len(m.evidence) for m in all_matches if m.pattern_type == pt)
        ...
        # ── Summary ───────────────────────────────────────────────────────────
        high_count = sum(
            1 for m in all_matches if m.confidence in (Confidence.HIGH, Confidence.STRUCTURAL)
        )
...
def _build_cli_parser():
    ...
    parser.add_argument(
        "--output", "-o", default=None, help="Write output to this file (default: stdout)"
    )
    return parser
...
def main(argv=None):
    ...
    result = analyze_file(args.input, tradition=args.tradition, output_format=args.format)
    if args.output:
