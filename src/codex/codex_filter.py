...
class PassageAnalysis:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "passage_preview": self.passage_text[:120]
            + ("..." if len(self.passage_text) > 120 else ""),
...
class InstitutionalFilter:
    ...
    def _analyze_passage(
        ...
        if level == CoherenceLevel.PURE_SIGNAL:
            ...
            note = (
                "Good signal strength — primary TIEKAT patterns present with minor filter presence."
            )
    ...
    def _build_recovery_map(
        ...
        pure = [
            p.passage_index for p in passages if p.coherence_level == CoherenceLevel.PURE_SIGNAL
        ]
        strong = [
            p.passage_index for p in passages if p.coherence_level == CoherenceLevel.STRONG_SIGNAL
        ]
        mixed = [p.passage_index for p in passages if p.coherence_level == CoherenceLevel.MIXED]
        filtered = [
            p.passage_index for p in passages if p.coherence_level == CoherenceLevel.FILTERED
        ]
        inst = [
            p.passage_index for p in passages if p.coherence_level == CoherenceLevel.INSTITUTIONAL
        ]
    ...
    def analyze(
        ...
        passages = [self._analyze_passage(p, i) for i, p in enumerate(raw_passages)]
        ...
        overall_coherence = (
            sum(p.coherence_score for p in passages) / len(passages) if passages else 0.60
        )
        coherence_level = _coherence_level(overall_coherence)

        # Dominant layers
        filter_layer_names = [
            LayerType.AUTHORITY_CLAIM.value,
            LayerType.FEAR_FRAMING.value,
            LayerType.EXCLUSIVITY_MARKER.value,
            LayerType.DOCTRINAL_FORMULA.value,
            LayerType.TEMPORAL_POWER.value,
            LayerType.GENDER_ERASURE.value,
        ]
        signal_layer_names = [
            LayerType.PRIMARY_SIGNAL.value,
            LayerType.SOVEREIGN_VOICE.value,
        ]
        ...
        # Summary
        seam_str = (
            f"{len(seams)} editorial seam(s) detected" if seams else "no editorial seams detected"
        )
        summary = (
            f"Institutional filter analysis of {self.tradition_label} text "
            f"({word_count} words, {len(passages)} passages). "
...
def filter_file(
    ...
    return filter_text(text, tradition=tradition, source=str(p), output_format=output_format)
...
def main(argv=None):
    ...
    parser.add_argument("--input", "-i", required=True, help="Path to text file to analyze")
    parser.add_argument(
        "--tradition",
        "-t",
        default="generic",
    ...
    result = filter_file(args.input, tradition=args.tradition, output_format=args.format)
    if args.output:
